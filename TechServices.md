...
Some OPACs allow you to specify a branch where your holds will be delivered.  BiblioCommons also indicates when an item is available at your favorite branch(es).
...
Suppose you have library cards at two different libraries.  How does Hoopla handle that?  How do you find out what to do?  One important clue is that you need separate email addresses for the separate accounts.
...
The problem is that the "LGBTQIA+ (Fiction)" is only included in some of the items it applies to, with the result that search results will be incomplete.

##### But

Couldn't a patron just ask at the reference desk?  There are a couple of answers to that, starting with, "Hell, No!!".

Are you sure that the reference librarian understands enough about boolean searching to work through all the possibilities in that Venn Diagram?  How much training have they had on your catalog?

Assuming the patron is comfortable discussing sexuality with the reference librarian at all, what do you want the response to be:

* "Here's a list of books"
* "Here's how I used the catalog to search for these books"

or both, of course.  If a reference librarian can't help patrons learn about searching the catalog, who can?
...
of both." [99, p13].  To me, this is a  rejection of Library of Congress control in reaction to the time taken to respond to the "illegal aliens" heading.  Something other than, or in addition to, the Library of Congress change process is clearly needed.
...
FCPL doesn't have a full query language, so a patron can't write queries like "(crocodile OR alligator) AND smile".  A patron also can't build a query which will return the whole catalog, which makes it hard (or impossible?) to do a facet-only search like "show me all the Large Print books newest first".

A more significant issue is that there is no way to search for bilingual material [6].  There is a language facet, but you can only choose one language at a time, and it uses the MARC 008 leader field, which can only specify a single language.
