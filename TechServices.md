...
## Other Services Adding Value to Technical Services
...
I can think of two ways of doing quality control of the catalog data, and one that doesn't work very well.

The one that doesn't work very well is to check catalog records for internal consistency.  For example, you could check that if a catalog record has an 830 field, it also has a 490 field.  The problem is, this sort of technical error won't make any difference to patrons (although I suppose this example depends on how you have your OPAC configured) and errors that do make a difference won't be picked up by this sort of check.

One way that works pretty well — one I've used a few times in a couple of different ways — is to check catalog records against an external reference.
 * subjects and names against the LoC authority files
 * series information (or the lack of it) against GoodReads
